...
final class AppText {
    ...
    static String translate(Context context, String raw) {
        ...
        String exact = switch (raw) {
            ...
            case "选择图片离线识别，或粘贴课程文字；确认预览后才会写入" -> "Recognize an image offline or paste schedule text. Nothing is saved until you confirm the preview";
            case "从相册选择课程表图片并离线识别" -> "Choose a schedule image and recognize it offline";
            case "选择课程表图片" -> "Choose schedule image";
            case "正在离线识别…" -> "Recognizing offline…";
            case "识别与课程文字" -> "Recognized schedule text";
            case "选择图片后会在这里显示识别结果，也可以直接粘贴并修改" -> "Recognized text appears here. You can also paste or edit it";
            case "图片识别在本机完成。请先修正分组、错字和星期，再生成预览" -> "Recognition runs on this device. Fix groups, typos, and weekdays before previewing";
            case "图片中没有识别到文字，请换一张更清晰、正对课表的图片" -> "No text was found. Try a clearer image taken straight above the timetable";
            case "图片识别失败，请换一张图片重试" -> "Image recognition failed. Try another image";
            case "无法读取这张图片，请换一张图片重试" -> "This image cannot be read. Try another image";
            case "识别完成，请先检查文字再生成预览" -> "Recognition complete. Check the text before previewing";
            ...
            case "没有识别到课程，请检查星期和科目的排列" -> "No lessons found. Check the weekday and subject layout";
