...
- **Schedule Import:** 课表页将“批量导入”和“设置节次时间”并列为次要操作。导入底页优先提供系统图片选择器，以内置中文模型在本机识别课表，并按照星期列和节次行重建文字；也接受手动粘贴。识别结果先落入可编辑文字区，随后才进入可滚动预览。默认跳过已有节次，只有用户明确打开替换开关才覆盖。新识别科目进入科目库但保持“尚未设置携带物”状态。识别中禁用图片操作并显示明确状态；失败必须保留现有输入并给出重试方式。
