...
首次进入采用独立的三步全屏流程：价值说明、导入课表、逐件打卡。每步只保留一个主标题、一段说明和一个明确动作，提供“稍后设置”，并以横向共享轴动画表达前进与返回；最后一步直接进入真实的课表导入，而不是演示模式。现有用户升级不重新经历首次流程，只显示一次版本更新底页。更新底页使用版本标记与连续功能列表，只写用户可感知变化，不出现 Action、构建或内部维护内容。
