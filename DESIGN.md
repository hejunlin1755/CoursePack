...
- **Primary:** 动态主色填充、22dp 圆角、粗体标签；普通按压只短暂收紧并提高圆度。首次引导的 64dp 主操作是唯一例外：随步骤在圆形、圆角方形与单侧高圆角叶片形之间连续变形，外壳每页旋转 360°，图标以等量反向旋转保持正立；不能用整颗按钮缩扁后旋转来冒充形状变换。
...
- **Schedule Import:** 课表页将“批量导入”和“设置节次时间”并列为次要操作，两个独立按钮之间必须至少保留 16dp 间距，任何尺寸都不得零间隙粘连。导入底页的图片选择按钮与下方识别说明同样至少保留 16dp 分组间距。导入底页优先提供系统图片选择器，以内置中文模型在本机识别课表，并按照星期列和节次行重建文字；也接受手动粘贴。识别结果先落入可编辑文字区，随后才进入可滚动预览。默认跳过已有节次，只有用户明确打开替换开关才覆盖。新识别科目进入科目库但保持“尚未设置携带物”状态。识别中禁用图片操作并显示明确状态；失败必须保留现有输入并给出重试方式。
...
首次进入采用独立的四步全屏流程：价值说明、通知权限、提醒时间、完成。通知页先说明“仅在清单未完成时提醒”，再由主按钮触发系统权限面板；通知和提醒均可跳过。欢迎插画使用透明书包前景与独立圆角容器，不能把 adaptive icon 的矩形底板直接绘制到普通 ImageView。页面用横向共享轴维持方向，插画的底板、主体与信息块分层错峰进入。底部使用高位表面承载左侧动作文字和右侧独立图标按钮；文字前进时由下向上交接，返回时方向相反。图标按钮四角以 600ms 在圆形、圆角方形、叶片形间插值，外壳以 900ms 旋转 360°，内部箭头等量反向旋转而保持正立；最后一页的箭头以 90ms 退出，再延迟 90ms 用 220ms 淡入缩放为完成勾。动画被打断时必须从当前可见值续接，不得叠加旧动画。系统关闭动画时立即切换内容。最后一步直接进入真实课表导入。现有用户升级不强制重走首次流程，可从设置重新查看；更新底页只写用户可感知变化。
